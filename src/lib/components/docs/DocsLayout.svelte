...
<div class="min-h-screen bg-[#0F172A] text-slate-300 font-sans selection:bg-[#2563EB]/30">
	<!-- Top Navigation -->
		...
		<div class="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
			...
			<div class="flex items-center gap-6">
				<button 
					...
					class="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/[0.05] bg-[#0B1120]/50 text-slate-500 hover:text-slate-300 transition-all text-sm w-64"
				>
					<SearchIcon size={14} />
					<span>Search documentation...</span>
					<kbd class="ml-auto text-[10px] font-sans opacity-50 bg-[#111827] px-1.5 py-0.5 rounded border border-white/[0.05]">⌘K</kbd>
				</button>
				
				<div class="h-4 w-[1px] bg-white/[0.05] hidden md:block"></div>
	...
	<div class="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 flex gap-8">
		...
		<!-- Sidebar -->
		<aside 
			class="fixed inset-y-0 left-0 z-50 w-72 bg-[#111827] border-r border-white/[0.05] transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:z-0 lg:bg-transparent lg:border-none {isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}"
		>
		...
		<main class="flex-1 min-w-0 py-12 lg:py-16">
			<nav class="flex items-center gap-2 text-xs font-medium text-slate-500 mb-8" aria-label="Breadcrumbs">
				<a href="/docs" class="hover:text-[#2563EB] transition-colors">Docs</a>
			...
			<div bind:this={contentElement} class="prose prose-invert prose-slate max-w-none 
				...
				prose-a:text-slate-300 prose-a:font-medium prose-a:no-underline hover:prose-a:text-[#2563EB] transition-colors
				prose-strong:text-white prose-strong:font-bold
				prose-code:text-slate-300 prose-code:bg-[#0B1120] prose-code:border prose-code:border-white/[0.05] prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded-md prose-code:before:content-none prose-code:after:content-none
			...
			<footer class="mt-20 pt-8 border-t border-white/[0.05] flex flex-col sm:flex-row justify-between items-center gap-4 text-slate-500 text-sm">
